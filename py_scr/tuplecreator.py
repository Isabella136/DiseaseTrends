...
country = lambda key, value : "INSERT INTO Country VALUES (" + ",".join(key) + "," + ",".join(value) + ");\n"
...
disease_info = lambda key, value : "INSERT INTO DiseaseInfo VALUES (" + ",".join(key) + "," + ",".join(value) + ");\n"

# {[CountryCode, Year, IndicatorCode, Sex, Causes, Severity, AgeGroup] : 
#  [LikelyValue, LowerValue, HigherValue]}
disability_adjusted_life_years_tuples = dict()
disability_adjusted_life_years = lambda key, value : "INSERT INTO DisabilityAdjustedLifeYears VALUES (" + ",".join(key) + "," + ",".join(value) + ");\n"
...
cases_tuples = dict()
cases = lambda key, value : "INSERT INTO Cases VALUES (" + ",".join(key) + "," + ",".join(value) + ");\n"
...
#   LikelyValue_Numeric, LowerValue_Numeric, HigherValue_Numeric]}
deaths_tuples = dict()
deaths = lambda key, value : "INSERT INTO Deaths VALUES (" + ",".join(key) + "," + ",".join(value) + ");\n"


# {[CountryCode, Year, IndicatorCode, Sex, Causes, Severity, AgeGroup] : [Rates]}
vaccines_tuples = dict()
vaccines = lambda key, value : "INSERT INTO Vaccines VALUES (" + ",".join(key) + "," + ",".join(value) + ");\n"
    ...
    with open("datasets/WHO/" + dataset) as dataset_file:
        ...
        for row_num, row in enumerate(dataset_reader):
            ...
            if row[6] not in country_to_region_dict:
                country_to_region_dict.update({row[6]:[row[4], row[3], False]})
            ...
            true_or_false ="'F'" if row[10] == 'false' else "'T'"
            ...
            elif "HIV-positive" in row[1]:                                population_group = "'HIV positive'"
            ...
            if ("death" in row[1]) or ("dying" in row[1]):
                deaths_tuples.update({("'" + row[6] + "'", row[9], "'" + row[0] + "'", sex, causes, severity, agegroup) :
                                      [per_expression_value, "'" + row[22] + "'", "'" + row[25] + "'", 
                                       "'" + row[27] + "'", row[23], row[26], row[28]]})
            elif "DALYs" in row[1]:
                disability_adjusted_life_years_tuples.update({("'" + row[6] + "'", row[9], "'" + row[0] + "'", sex, 
                                                               causes, severity, agegroup) :
                                                              [per_expression_value, incidence_vs_prevalence, 
                                                               row[23], row[26], row[28]]})
            elif "vaccine" in row[1]:
                vaccines_tuples.update({("'" + row[6] + "'", row[9], "'" + row[0] + "'", sex, causes, severity, agegroup) : 
                                        [row[23]]})
            else:
                cases_tuples.update({("'" + row[6] + "'", row[9], "'" + row[0] + "'", sex, causes, severity, agegroup) :
                                     [per_expression_value, incidence_vs_prevalence, 
                                      "'" + row[22] + "'", "'" + row[25] + "'", "'" + row[27] + "'", 
                                      row[23], row[26], row[28]]})
        ...
        if row[1] in country_to_region_dict.keys():
            for index, year in enumerate(year_list):
                country_tuples.update({("'" + row[1] + "'", year) : 
                                       ["'" + row[0] + "'", row[4+index] if row[4+index] != "" else "NULL", 
                                        "'" + country_to_region_dict[row[1]][0] + "'",  
                                        "'" + country_to_region_dict[row[1]][1] + "'"]})  
                country_to_region_dict[row[1]][2] = True    

with open("DiseaseTrendsInputData.sql", 'w') as SQL_file:
    for key, value in country_tuples.items():
